...
CREATE TABLE Flights(
    ...
    arr_airport CHAR(3) DEFAULT 'SOF' NOT NULL REFERENCES Airports(code),
...
-- 1. Find all flights departing from Sofia (`SOF`) with a flight duration longer than 90 minutes. 
-- Include airline name, flight number, departure time, and duration.
SELECT 
    f.fnumber AS Flight_Number,
    a.name AS Airline,
    f.flight_time AS Departure_Time,
...
JOIN 
    Airlines a
ON 
    f.airline_operator = a.code
WHERE 
    f.dep_airport = 'SOF'
    AND f.flight_duration > 90;


-- 2. List all customers who have booked flights through agencies in Sofia (`CITY = 'Sofia'`), 
-- including customer name, agency name, and flight number.
SELECT 
...
-- 3. Find the total revenue (sum of prices) generated by each airline. 
...
-- 4. Display all flights that have at least one booking, along with the total number of bookings for each flight.
SELECT 
    f.fnumber AS Flight_Number,
    COUNT(b.code) AS Total_Bookings
FROM 
    Flights f
LEFT JOIN 
    Bookings b
ON 
    f.fnumber = b.flight_number
GROUP BY 
    f.fnumber
HAVING 
    COUNT(b.code) > 0;
...
-- 5. Find the most popular destination airport based on the number of bookings.
SELECT 
    a.name AS Destination_Airport,
    COUNT(b.code) AS Booking_Count
FROM 
    Bookings b
JOIN 
    Flights f
ON 
    b.flight_number = f.fnumber
JOIN 
    Airports a
ON 
    f.arr_airport = a.code
GROUP BY 
    a.name
ORDER BY 
    Booking_Count DESC
...
-- 6. List all agencies that have booked flights on behalf of customers flying with "Bulgaria Air" (`FB`). 
-- Include agency name and the count of such bookings.
SELECT 
...
-- 7. Find all customers who have booked flights but have not flown yet (`STATUS = 0`). 
-- Include customer name, booking code, and flight date.
SELECT 
    c.fname || ' ' || c.lname AS Customer_Name,
...
-- 8. Calculate the average price of a flight booking for each airline.
SELECT 
    a.name AS Airline,
